...
class BouncingBallSimulation:
    ...
    def reset_game(self) -> None:
        ...
        print(f"Game reset! Final score: {self.game_state.score}")
        ...
        for ball in self.balls[:]:
            for cloud in self.clouds:
                if (cloud.x <= ball.x <= cloud.x + cloud.width and
                    ...
                    # Ball hits cloud - reduce speed significantly
                    ball.vx *= 0.3
                    ball.vy *= 0.3
    ...
    def _update_particles(self, dt: float) -> None:
        ...
        for particle in self.particles:
            particle.x += particle.vx * dt
            particle.y += particle.vy * dt
            particle.vy += 200 * dt  # Gravity for particles
            particle.life -= dt
    ...
    def _update_screen_shake(self, dt: float) -> None:
        ...
        if self.screen_shake_timer > 0:
            ...
            if self.screen_shake_timer > 0:
                intensity = (self.screen_shake_timer / ScoringConstants.SCREEN_SHAKE_DURATION) * ScoringConstants.SCREEN_SHAKE_INTENSITY
    ...
    def _award_points(self, x: float, y: float, points: int, 
                     color: Tuple[int, int, int] = Colors.BLACK,
                     font_size: int = 36, show_particles: bool = False) -> None:
        """No longer used in bird shooting game."""
        pass
    ...
    def _award_collision_points(self, x: float, y: float) -> None:
        """Disabled for minimal design."""
        pass
    
    def _award_air_time_points(self, ball: Ball, air_time: float) -> None:
        """Disabled for minimal design."""
        pass
    
    def _check_speed_achievements(self, ball: Ball) -> None:
        """Disabled for minimal design."""
        pass
    
    def _check_trick_shots(self, ball: Ball, current_time: float) -> None:
        """Disabled for minimal design."""
        pass
    
    def _award_wall_rider_points(self, ball: Ball) -> None:
        """Award points for consecutive wall bounces.
        
        Args:
            ball: Ball that hit wall
        """
        if ball.consecutive_wall_bounces >= 3:  # Award after 3+ consecutive bounces
            points = ScoringConstants.WALL_RIDER_POINTS * ball.consecutive_wall_bounces
            self._add_floating_text(ball.x, ball.y - 30, f"WALL RIDER x{ball.consecutive_wall_bounces}!", 
                                  Colors.ACHIEVEMENT_TEXT, 36)
            self._award_points(ball.x, ball.y, points, Colors.ACHIEVEMENT_TEXT, 24)
    ...
    def _check_pipe_entry(self, ball: Ball) -> None:
        ...
        if (pipe_x <= ball.x <= pipe_x + pipe_width and 
            ...
            self._award_points(ball.x, ball.y, points, Colors.ACHIEVEMENT_TEXT, 
                             36, show_particles=True)
            
            # "Swallow" the ball by removing it
            if ball in self.balls:
                self.balls.remove(ball)
                # Add swirl effect
                self._add_particles(ball.x, ball.y, count=20, color=Colors.PIPE_GREEN)
    ...
    def _check_slingshot_mastery(self, ball: Ball) -> None:
        ...
        if abs(launch_power - 1.0) < 0.05:  # 5% tolerance
            self._add_floating_text(ball.x, ball.y - 30, "PERFECT LAUNCH!", 
                                  Colors.ACHIEVEMENT_TEXT, 36)
            self._award_points(ball.x, ball.y, ScoringConstants.PERFECT_LAUNCH_POINTS, 
                             Colors.ACHIEVEMENT_TEXT, 24)
        
        # Gentle Touch: under 20% power that still scores
        elif launch_power < 0.2:
            self._add_floating_text(ball.x, ball.y - 30, "GENTLE TOUCH!", 
                                  Colors.ACHIEVEMENT_TEXT, 36)
            self._award_points(ball.x, ball.y, ScoringConstants.GENTLE_TOUCH_POINTS, 
                             Colors.ACHIEVEMENT_TEXT, 24)
        ...
        for ball in [ball1, ball2]:
            if (hasattr(ball, 'launch_power') and ball.launch_power > 0.5 and 
                ...
                self._add_floating_text(collision_x, collision_y - 40, "SNIPER SHOT!", 
                                      Colors.ACHIEVEMENT_TEXT, 48)
                self._award_points(collision_x, collision_y, ScoringConstants.SNIPER_POINTS, 
                                 Colors.ACHIEVEMENT_TEXT, 36, show_particles=True)
    ...
    def _draw_minimal_ground(self) -> None:
        """Draw a simple ground line at bottom."""
        ground_y = self.height - 5
        pygame.draw.line(self.screen, Colors.GROUND_COLOR, 
                        (0, ground_y), (self.width, ground_y), DisplayConstants.GROUND_THICKNESS)
    
    def _draw_minimal_ball(self, ball: Ball) -> None:
        """Draw ball with minimal styling.
        ...
        shadow_pos = (x + DisplayConstants.BALL_SHADOW_OFFSET, 
                     y + DisplayConstants.BALL_SHADOW_OFFSET)
        pygame.draw.circle(self.screen, Colors.GRAY, shadow_pos, radius)
        
        # Main ball (single color)
        pygame.draw.circle(self.screen, ball.color, (x, y), radius)
        
        # Minimal highlight (small, subtle)
        highlight_radius = radius // 4
        highlight_pos = (x - highlight_radius//2, y - highlight_radius//2)
        pygame.draw.circle(self.screen, Colors.WHITE, highlight_pos, highlight_radius)
    
    def _draw_minimal_slingshot(self) -> None:
        """Draw minimal slingshot visualization."""
        if not self.dragging_ball or not self.drag_start_pos:
            return
        
        start_x, start_y = self.drag_start_pos
        current_x, current_y = self.dragging_ball.x, self.dragging_ball.y
        
        # Simple line (no fancy styling)
        pygame.draw.line(self.screen, Colors.SLINGSHOT_LINE,
                        (int(start_x), int(start_y)), (int(current_x), int(current_y)), 2)
        
        # Minimal trajectory prediction
        self._draw_minimal_trajectory(start_x, start_y, current_x, current_y)
    
    def _draw_minimal_trajectory(self, start_x: float, start_y: float, 
                               current_x: float, current_y: float) -> None:
        """Draw simple trajectory line."""
        ...
        # Simple trajectory line (not dotted)
        trajectory_length = 100
        end_x = start_x + (dx / drag_distance) * trajectory_length
        end_y = start_y + (dy / drag_distance) * trajectory_length
        
        pygame.draw.line(self.screen, Colors.TRAJECTORY_LINE,
    ...
    def _draw_ui(self) -> None:
        """Draw status information and controls."""
        # Update FPS tracking
        self._update_fps()
        
        # Draw ball information
        # Draw scoring information
        self._draw_scoring_info()
        
        if self.balls:
            self._draw_ball_info(self.balls[0])
        
        # Draw controls and system info
        self._draw_controls()
    ...
    def _draw_ball_info(self, ball: Ball) -> None:
        """Draw information about the primary ball.
        
        Args:
            ball: Ball to display information for
        """
        # Move ball info to middle left to make room for scoring
        info_y_start = 150
        info_texts = [
            f"Position: ({ball.x:.1f}, {ball.y:.1f})",
            f"Velocity: ({ball.vx:.0f}, {ball.vy:.0f})",
            f"Speed: {ball.speed:.0f} px/s",
            f"Balls: {len(self.balls)}",
            f"Wall Bounces: {ball.consecutive_wall_bounces}"
        ]
        
        for i, text in enumerate(info_texts):
            surface = self.small_font.render(text, True, Colors.WHITE)
            y_pos = info_y_start + i * DisplayConstants.UI_LINE_HEIGHT
            self.screen.blit(surface, (DisplayConstants.UI_MARGIN, y_pos))
    ...
    def _draw_controls(self) -> None:
        ...
        controls = [
            ...
            "C - Add 3 balls",
            "Drag Ball - Slingshot",
            "Shift+Click - Add Ball",
            "",
            "Scoring Tips:",
            "• Hit birds for 500-1000pts",
            "• Chain collisions for combos",
            "• Aim for pipe (100/300pts)",
            "• Stay airborne for bonuses",
            "• Cloud zone = 2x multiplier"
        ]
        
        for i, control in enumerate(controls):
            color = Colors.CYAN if i == 0 else Colors.WHITE
            surface = self.small_font.render(control, True, color)
            x_pos = self.width - 150
            y_pos = DisplayConstants.UI_MARGIN + i * DisplayConstants.UI_LINE_HEIGHT
            self.screen.blit(surface, (x_pos, y_pos))
    
    def _draw_system_info(self) -> None:
        """Draw system performance information."""
        fps_text = f"FPS: {int(self._current_fps or self.clock.get_fps())}"
        fps_surface = self.small_font.render(fps_text, True, Colors.WHITE)
        fps_pos = (self.width - 80, self.height - 30)
        self.screen.blit(fps_surface, fps_pos)
    
    def _draw_pause_indicator(self) -> None:
        """Draw pause indicator overlay."""
        pause_surface = self.font.render("PAUSED", True, Colors.ORANGE)
        text_rect = pause_surface.get_rect(center=(self.width // 2, 50))
    ...
    def _start_slingshot(self, ball: Ball, mouse_x: float, mouse_y: float) -> None:
        ...
        # Store the ball's position as the slingshot anchor point
        self.drag_start_pos = (ball.x, ball.y)
    ...
    def _release_slingshot(self) -> None:
        ...
        # Get starting position (where we grabbed the ball)
        start_x, start_y = self.drag_start_pos
        
        # Get current ball position (where we dragged it to)
        current_x = self.dragging_ball.x
        current_y = self.dragging_ball.y
        
        # Calculate launch velocity (opposite to drag direction)
        # If you drag the ball down and right, it shoots up and left
        dx = start_x - current_x
        dy = start_y - current_y
    ...
    def _draw_slingshot(self) -> None:
        ...
        # Draw trajectory prediction based on actual launch calculation
        self._draw_trajectory_prediction(start_x, start_y, ball_x, ball_y)
        
        # Draw power meter
        self._draw_power_meter(start_x, start_y, ball_x, ball_y)
    ...
    def _draw_trajectory_prediction(self, start_x: float, start_y: float, 
        ...
        Args:
            start_x: Slingshot start X position
            start_y: Slingshot start Y position
            current_x: Current ball X position
            current_y: Current ball Y position
        """
    ...
    def _draw_zone_indicators(self) -> None:
        """Draw zone multiplier indicators."""
        # Cloud zone indicator (top)
        cloud_zone_y = ScoringConstants.CLOUD_ZONE_HEIGHT
        pygame.draw.line(self.screen, Colors.ZONE_INDICATOR, 
                        (0, cloud_zone_y), (self.width, cloud_zone_y), 2)
        
        # Ground zone indicator (bottom)
        ground_zone_y = self.height - ScoringConstants.GROUND_ZONE_HEIGHT
        pygame.draw.line(self.screen, Colors.ZONE_INDICATOR, 
                        (0, ground_zone_y), (self.width, ground_zone_y), 2)
        
        # Zone labels
        cloud_text = self.small_font.render("CLOUD ZONE (2x)", True, Colors.ZONE_INDICATOR)
        self.screen.blit(cloud_text, (10, cloud_zone_y - 20))
        
        ground_text = self.small_font.render("GROUND ZONE (0.5x)", True, Colors.ZONE_INDICATOR)
        self.screen.blit(ground_text, (10, ground_zone_y + 5))
    
    def _draw_scoring_info(self) -> None:
        """No longer used - replaced by simple bird shooting UI."""
        pass
